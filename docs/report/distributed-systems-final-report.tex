...
Kubernetes is the default deployment model: each component runs as one or more pods, while Services provide stable endpoints for internal and external traffic~\cite{burns2018designing}.
...
whereas asynchronous AMQP messaging decouples ingestion from execution between API and workers~\cite{fowler2014microservices}.
...
Fault tolerance in this system is based on \textbf{durable state}, \textbf{asynchronous retries}, and \textbf{component isolation}.
There is no separate replication layer for the application logic itself; instead, resilience comes from storing business data in \textbf{durable backends} and keeping the message broker as a \textbf{coordination layer} rather than as the source of truth.
PostgreSQL stores job metadata, state transitions, and processing outcomes, while MinIO stores the uploaded audio objects.
Because the persistent state is externalized, the API and worker services can restart independently without losing the information needed to resume processing.

Retry handling is applied to \textbf{transient failures} that can occur during processing or storage access.
If a worker cannot complete a job because of a temporary issue, the message can be retried and the job remains recoverable until the retry budget is exhausted.
This keeps the failure policy explicit and prevents the queue from silently discarding unfinished work.

Error handling is split across the components that can detect failures earliest.
The API rejects invalid uploads and authentication failures immediately, so only valid jobs enter the system.
The worker records processing failures in the job state, making them visible through the status and result endpoints and allowing operators to inspect persistent diagnostics.
In this way, faults are converted into \textbf{observable job states} rather than hidden internal errors.
...
The system does not rely on a dedicated \textbf{caching} layer.
This choice is deliberate: the most frequently accessed data are job metadata, processing state, and final results, all of which are already stored in durable backends and are small enough to retrieve directly.
Avoiding cache invalidation logic keeps the system simpler and reduces the risk of serving stale job information.

Availability is instead improved through \textbf{load balancing} and \textbf{horizontal replication of stateless services}.
In Kubernetes, the API can run with multiple replicas behind a Service, so incoming HTTP traffic is distributed across available instances.
Worker replicas also scale horizontally and consume from the same RabbitMQ queue, which makes processing capacity more resilient to spikes in demand or temporary pod restarts.

In case of \textbf{network partitioning}, the system degrades in a controlled way rather than hiding partial failures.
If the API cannot reach the broker or the storage backends, uploads fail fast and return an error instead of accepting jobs that cannot be completed.
If workers lose connectivity while processing, jobs remain persisted and can be retried once connectivity is restored.
This behavior favors consistency and recoverability over masking broken communication paths, which is appropriate for an asynchronous processing pipeline.


\subsection{Observability}\label{observability}

Observability is treated as a first-class design concern because the pipeline is asynchronous and spans multiple loosely coupled components.
The goal is not only to detect failures, but also to understand how requests move through the system, where work accumulates, and which component becomes a bottleneck under load.
For this reason, the observability solution is built around \textbf{metric exposure}, \textbf{centralized scraping}, and \textbf{dashboard-based visualization}.

\begin{figure}
  \centering
  \includegraphics[width=1.0\textwidth]{figures/observability-overview.png}
  \caption{Simplified three-layer observability view: workload components, Prometheus collection, and Grafana visualization.}
  \label{fig:observability-overview}
\end{figure}

A simplified view of the monitoring architecture is shown in \cref{fig:observability-overview}.
The first layer contains the main workload components of the pipeline (API, RabbitMQ, worker, PostgreSQL, and MinIO).
The second layer contains Prometheus, which collects metrics from those components.
The third layer contains Grafana, which visualizes the collected metrics through dashboards.

Each runtime component exposes Prometheus-compatible metrics that describe its own behavior.
The API reports request volume and latency, the worker reports processing activity and failure counts, and the queue layer exposes backlog-related signals that indicate whether processing capacity is keeping up with demand.
These metrics are aligned with the most relevant quality concerns of the project: responsiveness, throughput, reliability, and scaling behavior.

Prometheus acts as the collection layer by scraping the exposed metrics at regular intervals and storing them in a time-series format.
Grafana then consumes those metrics to provide a consolidated view of the system, making it easier to inspect trends over time and correlate events such as upload bursts, queue growth, and worker slowdowns.
This separation between metric production and visualization keeps the application code lightweight while still giving a clear operational picture.

From a design perspective, observability also supports validation and debugging.
When a job remains pending for too long or a worker begins failing repeatedly, the metrics make the problem visible before it becomes a silent functional error.
In that sense, the monitoring stack complements fault tolerance: fault tolerance preserves correctness under failure, while observability makes the failure itself measurable and actionable.
...
The API and frontend communicate over \textbf{HTTP}, while the worker is coordinated asynchronously through \textbf{AMQP} via RabbitMQ. In-transit data is represented mostly as \textbf{JSON} for API responses and queue messages, with \texttt{multipart/form-data} for uploads and form-encoded credentials for login. PostgreSQL is queried directly with \textbf{SQL} through \texttt{psycopg}, using a relational schema for users, jobs, and processing results.

Authentication uses \textbf{JWT} access tokens with \textbf{bcrypt} password hashing, and authorization relies on \textbf{RBAC} plus job ownership checks so users only access their own data. The implementation is built with \textbf{FastAPI}, \textbf{Uvicorn}, and \textbf{Poetry}; it also uses RabbitMQ, MinIO, Prometheus-compatible metrics, and Python's standard \texttt{wave} module to extract WAV audio features.
...
The source repository for the project is available at \url{https://github.com/matteo1610/distributed-audio-pipeline}.
...
Automatic testing is currently based on \textbf{unit and component-level tests} for the two Python services. The API test suite uses \texttt{FastAPI.TestClient} together with mocks to cover health and metrics endpoints, registration and login, upload handling, job status and result retrieval, and the main failure paths. The worker test suite uses mocked broker, storage, database, and metrics dependencies to verify message processing, retry handling, startup wiring, and database-readiness checks.

Concretely, the main automated test files are \path{srcs/app/tests/test_api.py}, \path{srcs/app/tests/test_auth.py}, and \path{srcs/worker/tests/test_worker.py}.

These tests are automated with \texttt{pytest} and focus on service logic in isolation, so they validate request handling, authentication flow, message consumption, and state transitions without relying on live infrastructure. The API tests can be run from \path{srcs/app} with \texttt{poetry run pytest}, and the worker tests can be run from \path{srcs/worker} with \texttt{poetry run pytest}.

In addition, the GitHub Actions workflow \path{.github/workflows/python-tests.yml} executes the test suites automatically on push and pull request events when files under \path{srcs/app/**}, \path{srcs/worker/**}, or the workflow file itself are modified. A full end-to-end suite is not automated yet; complete-stack verification is still done manually during deployment and smoke testing.
...
Deployment is provided in two reproducible modes: Docker Compose for local execution and kind+Helm for local Kubernetes execution.
The Compose path is the default for fast setup, while the Kubernetes path is used for orchestration-oriented experiments.

\subsection*{Required software and versions}

To run the project from scratch with containers, the machine needs:
\begin{itemize}
  \item Docker Engine and Docker Compose plugin (for local multi-container deployment);
\end{itemize}
...
\begin{itemize}
  \item PostgreSQL \texttt{16.13-bookworm};
  \item RabbitMQ \texttt{4.2.5-management};
  \item MinIO \texttt{RELEASE.2025-09-07T16-13-09Z-cpuv1};
  \item Prometheus \texttt{v2.55.1};
  \item Grafana \texttt{11.2.0};
  \item Python \texttt{3.12.13} for API and worker images.
\end{itemize}

\subsection*{From-scratch installation with Docker Compose}
...
Expected outcomes:
\begin{itemize}
  \item API reachable on \url{http://localhost:8000};
  \item Frontend reachable on \url{http://localhost:5173};
  \item RabbitMQ management UI on \url{http://localhost:15672};
  \item MinIO console on \url{http://localhost:9001}.
\end{itemize}
...
\begin{itemize}
  \item Prometheus on \url{http://localhost:9090};
  \item Grafana on \url{http://localhost:3000} (default \texttt{admin/admin}).
\end{itemize}

\subsection*{From-scratch installation with kind and Helm}

From \path{srcs}, create a local cluster and deploy the application chart:
\begin{verbatim}
...
\begin{itemize}
  \item API on \url{http://localhost:8000};
  \item Worker metrics on \url{http://localhost:9100/metrics};
  \item RabbitMQ management on \url{http://localhost:15672};
  \item MinIO API/console on \url{http://localhost:9000} and \url{http://localhost:9001};
  \item Prometheus and Grafana on \url{http://localhost:9090} and \url{http://localhost:3000}.
\end{itemize}

\begin{figure}
  \centering
  \includegraphics[width=1.0\textwidth]{figures/k8s-deployment/dap-k8s-deployment.png}
  \caption{Application chart deployment in kind (release \texttt{dap}).}
  \label{fig:k8s-dap-deployment}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[width=1.0\textwidth]{figures/k8s-deployment/obs-k8s-deployment.png}
  \caption{Observability chart deployment in kind (release \texttt{dap-observability}).}
  \label{fig:k8s-observability-deployment}
\end{figure}

\subsection*{Environment variables and configuration files}

The Compose deployment defines runtime variables directly in \path{srcs/docker-compose.yaml}.
For API local development, a reference template is provided in \path{srcs/app/.env.example}, covering database, broker, object storage, and API host/port settings.

In Kubernetes, configuration is managed through Helm values files:
\begin{itemize}
  \item \path{srcs/k8s/values-kind.yaml};
  \item \path{srcs/k8s/distributed-audio-pipeline/values.yaml};
  \item \path{srcs/k8s/observability/values-kind.yaml}.
\end{itemize}
These files define service types, NodePorts, image tags, credentials, and storage settings.

\subsection*{How deployment assets are engineered}

Deployment assets follow a \textbf{layered design}.
For Docker Compose, the base stack is defined in \path{srcs/docker-compose.yaml} (API, worker, PostgreSQL, RabbitMQ, MinIO, frontend), with health checks and service dependency conditions to enforce startup ordering.
Observability is added as a \textbf{compose overlay} in \path{srcs/observability/docker-compose.observability.yaml}, so monitoring can be enabled without modifying the base file.

For Kubernetes, the project uses \textbf{two separate Helm charts}: one for the application stack (\path{srcs/k8s/distributed-audio-pipeline}) and one optional chart for observability (\path{srcs/k8s/observability}).
This separation is intentional and fundamental: the \textbf{core system and observability infrastructure are decoupled}, allowing independent deployment, scaling, and lifecycle management.
The application can run without observability, and monitoring can be added or updated without touching the main workload.
To keep chart-embedded assets synchronized, the script \path{srcs/scripts/sync_k8s_assets.sh} copies SQL and observability configuration files into chart \path{files/} directories before release updates.

Container image delivery is \textbf{automated} through CI: a workflow builds and pushes updated API and worker images to Docker Hub whenever significant project changes are merged, and deployment configurations then \textbf{pull tagged images from Docker Hub} (instead of rebuilding on-cluster), keeping rollouts reproducible across environments and simplifying update propagation.

The Helm-based packaging also provides a \textbf{standard and repeatable deployment method across Kubernetes environments}.
While kind is used in this project mainly for local experimentation convenience, the same chart structure and values-driven configuration can be applied with minimal changes to any conformant Kubernetes cluster.
The default deployment profile is \textbf{intentionally basic} (single-replica settings and local-oriented resource choices) to keep execution lightweight and fast on a developer machine.
Even so, the current chart and service decomposition already support straightforward evolution toward multi-replica workloads, stronger distribution across nodes, and more production-oriented scaling policies.
...
The user-facing workflow is intentionally simple: authenticate, upload an audio file, monitor the job, and retrieve the extracted metadata once processing completes. The workflow is exercised through the lightweight frontend (see Figure~\ref{fig:frontend-placeholder}), the preferred interface for manual testing of backend functionality.

\begin{figure}
  \centering
  \includegraphics[width=1.0\textwidth]{figures/frontend-placeholder.png}
  \caption{Lightweight frontend used to upload files and inspect job status.}
  \label{fig:frontend-placeholder}
\end{figure}

\subsection*{Main Workflows}

\subsubsection*{1. Register and log in}

Create an account or log in with existing credentials to obtain a JWT access token.
This token is required for upload, status, and result requests.

\subsubsection*{2. Upload an audio file}

Use the frontend file picker or the API interface to submit an audio file.
The system stores the file in MinIO, creates a job record in PostgreSQL, and queues the processing request asynchronously.

\subsubsection*{3. Monitor job progress}

After upload, the job moves through the usual lifecycle states: \texttt{PENDING}, \texttt{PROCESSING}, and \texttt{DONE} or \texttt{FAILED}.
The frontend shows the current status and lets the user follow the processing progress in real time.

\subsubsection*{4. Retrieve the extracted metadata}

When the job reaches \texttt{DONE}, the user can open the result view to see the extracted audio metadata, such as duration, sample rate, and number of channels.

\subsection*{Observability}

If observability is deployed, you can run the provided setup and then inspect the collected metrics in Grafana.
Grafana is available at \url{http://localhost:3000} (default credentials: \texttt{admin/admin}).

The main dashboards summarize request rate, worker throughput, queue backlog, and the general health of the stack. An example Grafana dashboard is shown in Figure~\ref{fig:grafana-dashboard}; it highlights total uploads, completed jobs, failed jobs, and the 5-minute average upload latency, providing a quick operational view of pipeline health.
To populate these panels with sample data, the repository includes a simple stress script that uploads example files: \path{srcs/scripts/stress_uploads.sh}.

\begin{figure}
  \centering
  \includegraphics[width=1.0\textwidth]{figures/grafana-dashboard.png}
  \caption{Example Grafana dashboard showing total uploads, completed jobs, failed jobs, and 5-minute average upload latency.}
...
% It should be noted that each student is only responsible for their own section.% \begin{itemize}
%   \item An individual section is required for each member of the group
%   \item Each member must self-evaluate their work, listing the strengths and weaknesses of the product
%   \item Each member must describe their role within the group as objectively as possible.
% \end{itemize}
...
This work spans full-stack implementation from domain modeling and FastAPI backend with JWT authentication and RBAC, through background workers, to containerized infrastructure supporting dual deployment strategies: Docker Compose for local development and Kubernetes/Helm for orchestration.
The system achieves portability across environments while maintaining clear architectural boundaries between core services and optional monitoring layers.

Observability and monitoring are integrated from the start, with Prometheus metrics and Grafana dashboards providing visibility into request rate, job completion, and queue health.
Automated unit and component testing with CI/CD integration through GitHub Actions ensures code quality, and comprehensive technical documentation explains architectural decisions, implementation choices, and deployment procedures.
...
This system serves as a solid foundation for a production-grade distributed audio processing pipeline. Key areas for extension and enhancement include:

\begin{itemize}
  \item \textbf{Kubernetes and infrastructure}: Extend the Helm charts with scaling policies (e.g., autoscaling based on queue depth or worker CPU), resource limits, and production-oriented network policies. Document recommended configurations for multi-replica deployments and cloud-native environments.
  
  \item \textbf{Observability enhancements}: Add distributed tracing (e.g., OpenTelemetry) to track requests across service boundaries and simplify debugging. Implement granular logging for error cases and state transitions. Extend Grafana dashboards with more advanced metrics and alerting rules.
  
  \item \textbf{Frontend implementation}: Replace the minimal placeholder with a fully-featured web interface including real-time progress indicators, detailed error messages, job history, and rich state visualization. Consider progressive loading and offline support for improved user experience.
  
  \item \textbf{Integration testing}: Automate end-to-end testing using container-based test harnesses (e.g., TestContainers or Docker Compose-based e2e tests) to catch regressions early and improve overall reliability.
\end{itemize}
