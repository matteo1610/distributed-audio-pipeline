...
\begin{figure}
  \centering
  \includegraphics[width=0.78\textwidth]{figures/component-diagram.pdf}
  \caption{Kubernetes deployment view (logical mapping of API, worker, RabbitMQ, PostgreSQL, and MinIO as separate services/pods).}
  \label{fig:k8s-deployment-view}
\end{figure}
...
Deployment assets follow a layered design.
The base stack is defined in \path{srcs/docker-compose.yaml} (API, worker, PostgreSQL, RabbitMQ, MinIO, frontend), with health checks and service dependency conditions to enforce startup ordering.
Observability is added as a Compose overlay in \path{srcs/observability/docker-compose.observability.yaml}, so monitoring can be enabled without modifying the base file.

For Kubernetes, the project uses two Helm charts: one for the application stack (\path{srcs/k8s/distributed-audio-pipeline}) and one optional chart for observability (\path{srcs/k8s/observability}).
To keep chart-embedded assets aligned with canonical sources, the script \path{srcs/scripts/sync_k8s_assets.sh} copies SQL and observability configuration files into chart \path{files/} directories before release updates.
